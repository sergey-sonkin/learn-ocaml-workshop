...
let simpler_sum xs = List.fold xs ~init: 0 ~f:Int.(+)
...
let range from to_ = List.init (to_ - from) ~f:(Int.(+) from)
...
let print_int_list xs = List.iter xs ~f:(fun x -> Stdio.print_endline (Int.to_string x))
